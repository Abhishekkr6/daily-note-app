...
export function DailyReflection() {
            ...
            <Card className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 border-indigo-100 dark:border-indigo-900/50 shadow-sm relative overflow-hidden">
                ...
                <CardContent className="p-6 flex flex-col items-center text-center space-y-4 relative z-10">
                    ...
                    <Button
                        ...
                        disabled={loading}
                        className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white shadow-md transition-all hover:scale-105 active:scale-95"
                    >
                        {loading ? (
                            <>
    ...
    return (
        <Card className="bg-gradient-to-br from-background to-muted/30 border-border shadow-sm overflow-hidden relative">
            <CardHeader className="pb-2 border-b border-border/50 bg-muted/20">
                <CardTitle className="flex items-center gap-2 text-lg">
                    <Sparkles className="w-5 h-5 text-indigo-500" />
                    <span>Daily Reflection</span>
                    {data?.date && <span className="text-xs font-normal text-muted-foreground ml-auto border px-2 py-0.5 rounded-full">{data.date}</span>}
                </CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-5">
                ...
                {/* Suggestion */}
                {data?.suggestion && (
                    <div className="bg-indigo-50 dark:bg-indigo-950/30 p-3 rounded-lg border border-indigo-100 dark:border-indigo-900/50 flex gap-3 items-start">
